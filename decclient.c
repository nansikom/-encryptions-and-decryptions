...
int main(int argc, char *argv[]) {
 ...
 // char babymessage[45];
    // charsRead = recv(socketFD, babymessage, sizeof(babymessage),0);
    // if (charsRead < 0){
    //   error("Client reading from the enc_server socket");
    // }else if (charsRead == 0){
    //   fprintf(stderr,"Client: I think decclientis using enc_server and this is not allowed\n");
    //   exit(1);
    // }else{
    //   babymessage[charsRead] = '\0';
    //   fprintf(stderr, "%s\n", babymessage);
    //   exit(1);
    // }
  // Get input message from
...
if (strlen(realkey) < strlen(plaintextchars)){
  ...
  //printf("Clinet sending plain text chars: \"%s\"\n", plaintextSizestr);
   /*
  initiating data transerf protocol which means start sending the size of plaintext to the server.keep sending untill all of the text size has been sent.
  */
  int lengthsent=0;
...
//initiating data transfer protoocal which means send the actual plaintext to server using the socket filedescriptor wats ur sending its size and null terminator.
...
/*
socket fd is the socket file descriptermof that socket that am sending data
plaintext chars is the data am sending
plaintext size is the number of bytes am sending based off wat av alreadysent.
0 coz am not setting any flags.
*/
